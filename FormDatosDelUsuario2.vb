...
#End Region
    ...
    Dim archivoImagenNueva As String

    Dim Tumb As Image = Nothing
    Dim ImagenOriginal As Image
    Dim ImagenActual As Image
    Dim Ancho As Integer
    Dim Origen As Integer
    Dim Alto As Integer
    Dim UsuarioImagen As String
    Dim booCambioLaImagen As Boolean
    Dim SeProdujoUnCambio As Boolean

    Dim NumerodeTemporales As Integer
    Dim NombreLibre As Boolean
    Dim PedacitoString As String
    ...
    '    'MsgBox("Se ejecuta el codigo de ddu2", MsgBoxStyle.Information)
    '    Inicializa()
    ...
    '    'crea direcciones a los archivos de imagen 
    '    pathImagenUsuario = Form1.PathDatosDelUsuario & "\Imagen.jpg"
    '    pathImagenUsuarioTemp = Form1.PathDatosDelUsuario & "\temp00.jpg"
    '    Form1.UsuarioImagen = pathImagenUsuario

    '    NombreLibre = False
    '    NumerodeTemporales = 0

    '    While NombreLibre = False

    '        If File.Exists(pathImagenUsuarioTemp) = False Then
    '            NombreLibre = True
    '        Else
    '            NumerodeTemporales += 1
    '            PedacitoString = CType(NumerodeTemporales, String)
    '            If PedacitoString.Length = 1 Then
    '                PedacitoString = "0" & PedacitoString
    '            End If
    '            pathImagenUsuarioTemp = Form1.PathDatosDelUsuario & "\temp" & PedacitoString & ".jpg"
    '        End If
    '    End While
    ...
    '        File.Copy(pathImagenUsuario, pathImagenUsuarioTemp, True)

    '        ImagenOriginal = Image.FromFile(pathImagenUsuarioTemp)
    '        ImagenActual = CType(ImagenOriginal.Clone(), Image)


    '        If ImagenActual.Height > ImagenActual.Width Then
    '            'es mas alta que ancha
    '            '48x80
    '            PictureBox1.Height = 80
    '            Ancho = Int(ImagenActual.Width * 80) \ ImagenActual.Height
    ...
    '            Alto = Int(ImagenActual.Height * 48) \ ImagenActual.Width
    ...
    '        Button1.Text = "Salir"
    '    Else
    '        'no existe hay que crear el archivo de imagen al menos poner uno para q luego no de error
    '        'selecciona la imagen por defecto (la embarazada) como la nueva imagen seleccionada
    '        pathImagenNuevaSeleccionada = Form1.PSstrDirectorioDeLaAplicacion & "\Embarazada.jpg"
    '        'copia el archivo al dir del usuario aunque al final se cancele la intro de datos del usuario
    '        File.Copy(pathImagenNuevaSeleccionada, pathImagenUsuario, True)
    '        'se dejan las dos bandera como si no se hubiese hecho ningun cambio
    '        SeCambioLaImagen = False
    '        SeProdujoUnCambio = False
    '        Button1.Text = "Salir"
    ...
    Private Sub RevisaSiExisteElArchivo()
        ...
        Else
            'no existe el archivo de datos del usuario inicializa las variables
            Form1.UsuarioNombre = ""
            Form1.UsuarioEspecialidad = ""
            Form1.UsuarioDir01 = ""
            Form1.UsuarioDir02 = ""

        End If

        'acomoda la pantalla para reflejar lo que se tiene
        txtNombre.Text = Form1.UsuarioNombre
        txtEspecialidad.Text = Form1.UsuarioEspecialidad
        txtDir01.Text = Form1.UsuarioDir01
        txtDir02.Text = Form1.UsuarioDir02
        ...
        'carga lo que sea q este en la memoria de logo sea una imagen o este imgenvacia
        ImagenActual = Form1.imagenLogoDelUsuario

        'acomoda la imagen actual a lo q se tiene
        If ImagenActual.Height <> 0 Then
            'si hay algo en la imagen lo procesa para crear el tumble
            If ImagenActual.Height > ImagenActual.Width Then
                'es mas alta que ancha
                '48x80
                PictureBox1.Height = 80
        ...
        'como sea asigna la imagen actual a picture box
        PictureBox1.Image = ImagenActual
        'acomoda banderas generales

        booCambioLaImagen = False
        SeProdujoUnCambio = False
        Button1.Text = "Salir"
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        If Form1.booSeDebeActualizarImagenDelUsuarioAlCerrar = True And SeProdujoUnCambio = True Then
            If MsgBox("Deseas revertir los cambios de imagen y membrete y mostrar lo que había al cargar la Base de Datos?", MsgBoxStyle.YesNo, "Confirmar Descarcar Cambios") = MsgBoxResult.Yes Then
                If File.Exists(Form1.ArchivoImagenDelUsuarioTemporal) Then
                    'se debe revertir los cambios y sacar los datos del temp que estaba originalmente
                    Form1.imagenLogoDelUsuario = Image.FromFile(Form1.ArchivoImagenDelUsuarioTemporal)
                Else
                    'no habia un temporal q preñao cancelo despues de la 2da vez q cargaba la vaina con este logo creando labase hace 1 arranque es la unica forma q pase por aqui
                    Form1.imagenLogoDelUsuario = FormImagenes.picImagenVacia.Image
                End If

                'apagar la bandera de q se debe cambiar la imagen
        ...
        ElseIf Form1.booSeDebeActualizarImagenDelUsuarioAlCerrar = False And SeProdujoUnCambio = True Then


            If MsgBox("Deseas revertir los cambios realizados al membrete?", MsgBoxStyle.YesNo, "Confirmar Descartar Cambios del Membrete") = MsgBoxResult.Yes Then
                Me.DialogResult = Windows.Forms.DialogResult.Cancel
            Else
                Exit Sub
            End If

        ElseIf Form1.booSeDebeActualizarImagenDelUsuarioAlCerrar = True And SeProdujoUnCambio = False Then

            If MsgBox("Deseas revertir los cambios de imagen y mostrar el Logo que había al cargar la Base de Datos?", MsgBoxStyle.YesNo, "Confirmar Descarcar Cambios de Logo") = MsgBoxResult.Yes Then
                'se debe revertir los cambios y sacar los datos del temp que estaba originalmente
                If File.Exists(Form1.ArchivoImagenDelUsuarioTemporal) Then
                    'se debe revertir los cambios y sacar los datos del temp que estaba originalmente
                    Form1.imagenLogoDelUsuario = Image.FromFile(Form1.ArchivoImagenDelUsuarioTemporal)
        ...
        Else
            ...
            'no han habido cambios sale sin problemas
            Me.DialogResult = Windows.Forms.DialogResult.Cancel
    ...
    Private Sub VerificaOk()
        If labNombre.Text <> "" And labEspecialidad.Text <> "" And labDir.Text <> "" Then
            ...
            If SeProdujoUnCambio = True Or Form1.booSeDebeActualizarImagenDelUsuarioAlCerrar = True Then
    ...
    Private Sub botImagen_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles botImagen.Click
        ...
        If OpenFileDialog1.ShowDialog = Windows.Forms.DialogResult.OK Then
            'se seleccionó una imagen nueva (cambio)
            Form1.ArchivoImagenDelUsuarioNuevo = OpenFileDialog1.FileName 'carga la direccion del archivo para posteriooooooooooooooormente guardarlo 
            Form1.imagenLogoDelUsuario = Image.FromFile(OpenFileDialog1.FileName) 'carga la imagen seleccionada como el logo del usuarios de una

            ImagenActual = Form1.imagenLogoDelUsuario 'carga la imagen en la variable local para mostrar como va quedando
            ...
            'prende la bandera de q se debe guardar la imagen al final
            Form1.booSeDebeActualizarImagenDelUsuarioAlCerrar = True
